**************Resource Sharing Challenge Project  Solution*************************************************

DESCRIPTION: This is the solution to the resouce sharing website 

Extract the files from cov-aid.zip.

_____________________________________________Folder Structure__________________________________________________
	
	
	project_solution/
	├── css/ (contains all the css files)
	│    ├── navstyle.cs
	│    └── style.css
	├──  img/
	│    └── (contains all images required for developing the website)
	├──  includes/	 
	│    ├── connection.php
	│    ├── footer.php
	│    └── header.php
	├──  contact.php
	├──  contact_script.php
	├──  cov-aid.sql (Database of the project)
	├──  deficit.php 
	├──  details.php
	├──  index.php
	├──  listing.php
	├──  listing_script.php
	├──  login.php
	├──  login_submit.php
	├──  logout.php
	├──  signup.php
	├──  signup_script.php
	├──  splash.html
	├──  stats.php
	├──  success.php
	├──  surplus.php
	├──  surplus_script.php
	└──  user_edit_details.php

____________________________________________SETUP_________________________________________________________________________________________				
1.) Start the Apache and MySQL modules using the XAMPP controller.
2.) Open the phpMyAdmin and create a database "cov-aid". 
3.) Import the cov-aid.sql file present in the zip folder.
4.) Open the htdocs folder in the xampp folder. Copy paste the folder cov-aid.
5.) Open the browser (chrome), type localhost/cov-aid/index.php and you should see the main page of the website.

*******************************************************************************************************************************************
